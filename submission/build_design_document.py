...
def build_document():
    ...
    add_body(doc, "算法采用 1 秒滑动窗口、123 个整数时域特征和 120 棵随机森林。板端加入分类别置信阈值、连续窗口确认、全局冷却和抬腕状态机，减少误报与重复输出。")
    ...
    add_bullet(doc, "投票阈值：pinch 93/120，clench 88/120，up/down 65/120。")
    add_bullet(doc, "同一类别连续 3 个窗口满足条件后才确认。")
    add_bullet(doc, "有效输出后进入 600 ms 全局冷却；同类阻塞需要连续 2 个非同类窗口后释放，抑制单动作重复输出。")
    ...
    for text in [
        ...
        "algo_task 提取特征、执行 120 棵树、运行状态机。",
        "识别有效手势后打印累计处理时间和类别。",
    ]:
        add_number(doc, text)
    ...
    add_body(doc, "六用户留一事件级宏 F1 为 93.79%。六折窗口准确率范围为 91.02% 至 93.85%。正确事件平均输出延迟约 313 ms。")
