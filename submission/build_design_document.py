...
def build_document():
    ...
    add_body(doc, "算法采用 1 秒滑动窗口、123 个整数时域特征和 100 棵随机森林。板端加入分类别置信阈值、连续窗口确认、全局冷却和抬腕状态机，减少误报与重复输出。")
    ...
    add_bullet(doc, "投票阈值：pinch 82/100，clench 77/100，up/down 50/100。")
    ...
    for text in [
        ...
        "algo_task 提取特征、执行 100 棵树、运行状态机。",
        "识别有效手势后打印累计处理时间和类别。",
    ]:
        add_number(doc, text)
    ...
    add_body(doc, "六用户留一事件级宏 F1 为 93.30%。六折窗口准确率范围为 90.84% 至 93.68%。正确事件平均输出延迟约 310 ms。")
