...
def build_document():
    ...
    add_bullet(doc, "投票阈值：pinch 40/50，clench 30/50，up/down 32/50。")
    add_bullet(doc, "同一类别连续 3 个窗口满足条件后才确认。")
    add_bullet(doc, "有效输出后进入 1600 ms 全局冷却，抑制单动作重复输出。")
    ...
    add_table(
        ...
        [
            ("pinch", 2210, 430, 570, "83.71%", "79.50%", "81.55%"),
            ("clench", 2274, 210, 504, "91.55%", "81.86%", "86.43%"),
            ("up", 2206, 21, 185, "99.06%", "92.26%", "95.54%"),
            ("down", 2203, 12, 183, "99.46%", "92.33%", "95.76%"),
        ],
        [1300, 850, 850, 850, 1500, 1500, 1510],
    )
    add_body(doc, "六用户留一事件级宏 F1 为 89.82%。六折窗口准确率范围为 90.58% 至 93.52%。正确事件平均输出延迟约 342 ms。")
